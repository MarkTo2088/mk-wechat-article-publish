...
让 AI 助手**帮你写公众号，并发布到草稿箱**（不群发）。你说选题或贴素材，助手起草、你确认后再发出。

稿件内部是 Markdown，转换只是发布步骤，不是这个工具的定位。支持品牌色排版、横向滑动画廊、小程序码，以及按工作空间配置多公众号。可作为 AI 助手的 Skill 安装，也可直接跑脚本。
...
之后直接说要写的主题，或把素材、提纲贴给助手。已有成稿也可以把文件路径发给它。

---

## 三、让助手写并发布

对助手说选题、受众和要点，或直接贴素材。助手会起草一篇稿，你改到满意后，它再预览并写入公众号草稿箱。

已有成稿时，把文件路径发给助手即可，不必重写。

要参考某篇已发布文章的排版时，把链接发给助手。它会提炼配色、标题节奏、引用和配图方式，写入本工作空间的 `.mk-wechat-publish/layout-ref.json`，再按这个参考写新稿，不会把原文抄过来。

助手生成的配图，发布前会逐张检查水印。有角标或「AI生成」字样的不会直接发出：先换成无水印的图，或只裁掉自己生图的边角小标，核对后再发。

不想对话、只想自己跑脚本时，稿件仍按下面格式保存（内部格式，不是产品定位）：
